...
/**
 * @brief Заполнение вектора и его вывод
 * 
 * @param element_number
 * номер элемента в последовательности Фибоначчи
*/
void fibonacci_output(const size_t& element_number)
{
    // Инициализация вектора, в котором будут храниться элементы последовательности
    std::vector<std::vector<unsigned int>> fibonacci_sequence_elements(element_number + 1);

    fibonacci_sequence(element_number, fibonacci_sequence_elements);

    // input_1-й элемент последовательности
    std::vector<unsigned int> element = fibonacci_sequence_elements[element_number];
